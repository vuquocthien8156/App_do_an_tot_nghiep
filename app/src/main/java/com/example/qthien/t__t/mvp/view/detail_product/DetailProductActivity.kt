...
import com.example.qthien.t__t.mvp.view.ealuation.CreateEvaluationActivity
import com.example.qthien.t__t.mvp.view.ealuation.EvaluationActivity
...
class DetailProductActivity : AppCompatActivity() , IDetailProduct ,
    ...
    override fun successCallDetail(response: ResponseEvaluation) {
        ...
        if(arrEvaluation.size == 0) {
            ...
            lnInfoEvaluation.visibility = View.VISIBLE
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setUpViewPager()
        checkFavorite()
        PreDetailProduct(this).callDetailOrEvaluation(product!!.idProduct , MainActivity.customer!!.idCustomer , null , null , null , null)
    ...
    fun addOrRemoveFavorite(){
        ...
        if (stringFavorite != null) {
            ...
            if(checkFavorite){
                ...
                for(i in 0 until arrFavorite.size){
                    Log.d("arrrFavoriteeeeeAdd" , arrFavorite.get(i))

                    if(arrFavorite.get(i).trim().equals(product?.idProduct.toString())) {
                        arrFavorite.removeAt(i)
    ...
    private fun setUpEvaluation() {
        ...
        btnReadMoreEvaluation.setOnClickListener({
            ...
            i.putExtra("nameProduct" , product?.nameProduct)
            i.putExtra("url" , product?.imageProduct)
            startActivity(i)
        })
    }
        ...
        if(requestCode == REQUEST_CART && resultCode == Activity.RESULT_OK) {
            ...
            snackbar.setAction(R.string.go_to_history_order, object : View.OnClickListener {
                override fun onClick(view: View) {
                    startActivity(Intent(this@DetailProductActivity , OrderHistoryActivity::class.java))
                }
            })
    ...
    override fun onOptionsItemSelected(item: MenuItem?): Boolean {
        when(item?.itemId){
            ...
            com.example.qthien.t__t.R.id.menu_share -> {
                ...
                i.putExtra(Intent.EXTRA_TEXT, "${RetrofitInstance.baseUrl}/products/productDetail?id=${product?.idProduct}")
