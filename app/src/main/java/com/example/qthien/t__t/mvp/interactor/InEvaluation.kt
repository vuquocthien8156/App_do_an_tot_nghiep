...
import com.example.qthien.t__t.mvp.presenter.detail_product.IPreEvaluation
...
class InEvaluation(var iPreEvaluation: IPreEvaluation) {
    ...
    val addImagesEvaluate = fun(arrFile : ArrayList<File>){
        ...
        for(file in arrFile){
            ...
            val body = MultipartBody.Part.createFormData("imgEv[]", file.absolutePath, requestBody)
        ...
        call.enqueue(object : Callback<ResponseMutilImage> {
            ...
            override fun onResponse(call: Call<ResponseMutilImage>, response: Response<ResponseMutilImage>) {
                val r = response.body()
                Log.d("adddddImg" , response.toString())
                Log.d("adddddImg" , response.toString())
                Log.d("adddddImg" , r?.imagesUrl.toString())
